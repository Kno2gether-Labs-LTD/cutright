...
def main():
    ...
    run(["python3",f"{SKILL}/captions_png.py","--cues",os.path.join(T,"cues.json"),"--outdir",capdir,
         "--concat",os.path.join(capdir,"concat.txt"),"--frame",f"{VW}x{VH}","--font",d["font"],"--end",str(span)])
    base=os.path.join(T,"base.mp4")
    vin=["-ss",str(A),"-t",str(span),"-i",src] if a.range else ["-i",src]
    ...
    if scenes:
        ...
        for s in scenes:
            ...
            run(["ffmpeg","-hide_banner","-y","-ss",str(s["start"]),"-t",str(s["dur"]),"-i",graded,
                 ...
                 "-map","[o]","-an","-c:v","h264_videotoolbox","-b:v","16M","-pix_fmt","yuv420p","-r",str(FPS),clip],log=os.path.join(T,f"clip_{s['id']}.log"))
            clips.append((s,clip))
