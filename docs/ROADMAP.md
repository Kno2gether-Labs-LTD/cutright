...
5. **The agent contract: `cutright` CLI + MCP server + skill** — the embedded `claude` calls `cut_range` / `add_overlay` / `set_look` / `find_moments` instead of hand-writing JSON, and gets structured results (and schema validation) back. Effort M.
